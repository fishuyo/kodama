...
class Tree() extends Animatable {
  ...
  def branch( n: TreeNode, depth:Int):Int = {
    ...
    var pos = n.pose.pos + n.pose.quat.toZ()*n.dist*n.scale
    ...
    for( i <- (0 until count) ){
      ...
      val node = TreeNode( Pose(pos,quat), dist, n.thick * n.taper, qb )
      node.scale = n.scale
...
class TreeNode extends Animatable {
  ...
  override def draw( ){ //v:Array[Float], idx:Int ):Int = {
		...
		TreeNode.model.scale.set(dist*thick*scale,dist*thick*scale,dist*scale) //*1.05f)
		TreeNode.model.draw();
    // var i = idx
  ...
  def solveConstraints() : Unit = {
    children.foreach( (n) => {
    	...
    	val pos = pose.pos + pose.quat.toZ()*dist*scale
    	n.pose.pos = pos
  ...
  def getMaxHeight():Float = {
    val p1 = pose.pos.y
    val p2 = (pose.pos + pose.quat.toZ()*dist*scale).y
